...
vec4 calcSpotLight(SpotLight spotLight, vec3 normal){
	...
	if (spotFactor > spotLight.range)
	
		color = calcPointLight(spotLight.pointLight, normal) * (1.0 - (1.0 - spotFactor)/(1.0 - spotLight.range));
...
void main() {
	...
	vec4 textureColor = texture(texture_sampler, textureCoord.xy);
