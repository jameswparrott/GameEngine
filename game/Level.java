...
//					addChild(wallObject);
				...
				if((level.getPixel(i, j) & 0x0000FF) == 16) {
					...
					boolean x = ((level.getPixel(i, j - 1) & 0xFFFFFF) == 0) && ((level.getPixel(i, j + 1) & 0xFFFFFF) == 0);
					
					boolean y = ((level.getPixel(i - 1, j) & 0xFFFFFF) == 0) && ((level.getPixel(i + 1, j) & 0xFFFFFF) == 0);
					...
					if (x ^ y) {
						...
						if(x) {
							...
							door.getTransform().setRot(new Quaternion(new Vector3D(0, 1, 0), (float) Math.toRadians(90.0f))); 
